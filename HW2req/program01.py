...
def generate_digits(bases : List[int] ) -> List[List[int]]:
    ...
    number_all_items = 1
    for elem in bases:
        number_all_items = number_all_items * elem
    number_card = number_all_items
    group_card = 1
    final_list = [[] for x in range(number_all_items)]
    for a in bases:
        i = 0
        number_card = number_card // a
        for m in range(group_card):
            for b in range(a):
                for n in range(number_card):
                    final_list[i].append(b)
                    i+=1
        group_card = group_card * a
    return final_list
...
def find_doubles(bases : List[int]) -> Set[int]:
    ...
    '''
    all_numbers_set = set()
    multiple_numbers_set = set()
    for digits_list in generate_digits(bases):
        n = decode_digits(digits_list,bases)
        if n not in all_numbers_set:
            all_numbers_set.add(n)
        else:
            multiple_numbers_set.add(n)
    return multiple_numbers_set
